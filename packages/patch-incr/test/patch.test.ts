...
import { _ensureRefTo } from "../patch/apply";
  ...
  describe("basic patch operations", () => {
    ...
    describe("PatchOp.Add", () => {
      it("should replace root through add", () => {
        expect(applyPatches(undefined, [{
          op: PatchOp.Add, path: [], value: "test"
        }])).toStrictEqual("test");
      });

      it("should add array element to the end", () => {
        expect(applyPatches([1, 2], [{
          op: PatchOp.Add, path: [IndexEnd], value: "test"
        }])).toStrictEqual([1, 2, "test"]);
      });

      it("should add array element in the middle", () => {
        expect(applyPatches([1, 2, 3], [{
          op: PatchOp.Add, path: [1], value: "test"
      ...
      it("should add array element in nested object", () => {
        expect(applyPatches({
          ...
          op: PatchOp.Add, path: ['a', 'b', 1], value: "test"
    ...
    describe("PatchOp.Remove", () => {
      it("should replace root through remove", () => {
        expect(applyPatches("test", [{
          op: PatchOp.Remove, path: [],
        }])).toStrictEqual(undefined);
      });

      it("should remove first element", () => {
        expect(applyPatches([1, 2], [{
          op: PatchOp.Remove, path: [0],
        }])).toStrictEqual([2]);
      });

      it("should remove last element", () => {
        expect(applyPatches([1, 2], [{
          op: PatchOp.Remove, path: [1],
        }])).toStrictEqual([1]);
      });
...
describe("_ensureRefTo", () => {
  it("should return ref to root", () => {
    const obj = { a: 1 };
    const [parent, key] = _ensureRefTo(obj, ["a"]);
    expect(key).toEqual("a");
    expect(parent).toBe(obj);
  });

  it("should return ref by path and shallow copy the values along the path except the root", () => {
    const inner = { x: { y : 1 } };
    const origB = { c: inner };
    const origA = { b: origB };
    const obj = { a: origA };
    const [parent, key] = _ensureRefTo(obj, ["a", "b", "c"]);
    expect(key).toEqual("c");

    expect((parent as { c: unknown }).c).toBe(inner);
    expect(obj).toStrictEqual({ a : { b: { c : { x: { y: 1 } }}}});
    expect(obj.a.b.c).toBe(inner);

    expect(obj.a).not.toBe(origA);
    expect(obj.a.b).not.toBe(origB);
    expect(obj.a.b.c).toBe(inner);
  });
})

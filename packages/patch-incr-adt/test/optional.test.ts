...
describe("optional", () => {
	describe("of record", () => {
		...
		it("should combine replaces", () => {
			expect(
				recordOpt.combine(
					recordOpt.fromReplace(defined),
					recordOpt.toUndefined,
				),
		...
		it("should combine inner changes", () => {
			...
			expect(recordOpt.combine(d1, d2)).toEqual(
				recordOpt.inner.combine(d1, d2),
			);
		});
		it("should replace from undefined to defined", () => {
			expect(
				recordOpt.apply(undefined, recordOpt.fromReplace(defined)),
			).toEqual(defined);
		});

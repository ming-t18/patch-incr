...
export interface ISetter<S extends $A, A extends $A> {
...
export interface ILens<S extends $A, A extends $A> extends ISetter<S, A> {
	readonly get: IFA<S, A>;
}

export interface ILens1<S extends $A, T extends $A, A extends $A, B extends $A>
	extends ISetter1<S, T, A, B> {
	readonly get: IFA<S, A>;
}

export interface IAffine<S extends $A, A extends $A> extends ISetter<S, A> {

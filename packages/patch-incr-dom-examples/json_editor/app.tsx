...
import { composeMemo, composeNoInterm } from "patch-incr/builder/compose";
import { entries } from "patch-incr/builder/struct/entries";
import { atomicFunc, constant, identity, recurse } from "patch-incr/builder";
...
const getRenderPair = (path0: Path, rec: IF<RenderValueProps, ElementConstruction>): IF<[string, unknown], ElementConstruction> =>
  switchCase(([, value]) => getKind(value), (kind) => {
    ...
    if (!(kind === KindArray || kind === KindObject)) {
      return template(
        ...
        ({ key, value }) => (
          <div data-key={key} class="jer-collection-element" style={componentStyle}>
            ...
            <div class="jer-collection-header-row">
              <div class="jer-collection-name">
                ...
                <span class="jer-key-text">
                  {key}
                  <span class="jer-key-colon">{':'}</span>
                  <span class="jer-brackets jer-bracket-open">{openClose[0]}</span>
                </span>
              </div>
            </div>
            {value}
...
const getRenderValue = (
  ...
  return recurse<IF<RenderValueProps, ElementConstruction>>((rec) => (
    bindMemo(getPath, (path0: Path): IF<RenderValueProps, ElementConstruction> => {
      const renderPair = getRenderPair(path0, rec);
      const renderObject: IF<Record<string, unknown>, ElementConstruction> = template(
        {
        ...
        ({ path, children }) => {
          return <div data-path={path} class="jer-component jer-collection-component" role="list">
            {children}
          </div>
        }
      );
      ...
      const renderList: IF<unknown[], ElementConstruction> = template(
        {
          children:
            composer(entries<string, unknown>())
              .pipe(map(renderPair))
              .build()
        },
        ({ children }) => {
          return <div class="jer-collection-inner" role="list">
            {children}
          </div>
        }
      );
      ...
      const renderAsString: IF<unknown, ElementConstruction> = template(
        {
          valueClass: atomicFunc((x: unknown) => `jer-value jer-value-${typeof x}`),
      ...
      return composeMemoL(
        ...
        switchCase(getKind, kind => {
          if (kind === KindArray) return renderList as IF<unknown, ElementConstruction>;
          if (kind === KindObject) return renderObject as IF<unknown, ElementConstruction>;
...
class JsonEditorComponent extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const root = this;
   	this.#domRoot = new DOMRoot(root, initState, editorReducer, renderEditor);
